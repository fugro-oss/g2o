...
Edge::Edge(const InformationType& information, const EdgeCallbacks& callbacks) :
...
{
  if (_callbacks.computeError(_callbacks.user, _error.data(), _dimension) != 0)
  {
    // The managed error function failed (e.g. threw an exception). Stop the
    // optimization as soon as possible; the managed layer rethrows afterwards.
    if (graph() != nullptr)
    {
